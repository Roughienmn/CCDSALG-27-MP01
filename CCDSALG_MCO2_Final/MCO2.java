...
public class MCO2 {
    ...
    public static void hashFunction(int algorithm, String sequence, int k)
    {
        ...
        int sameCollision = 0; // Number of collisions wherein the substring already exists in the hash table
        int indCollision = 0; // Number of collisions wherein the substrings gets a hash key already occupied
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            while(!found){
                ...
                // If it exists and is the same key, increment count (implying a possible collision):
                else if(hashtable[index].getName().equals(substring))
                {
                    hashtable[index].addCount();
                    found = true;
                    sameCollision++; // Counter
                }
                // If neither, update case for hashing function (linear probing) as it implies collision among two different substrings:
                else
                {
                    offset++;
                    indCollision++; // Counter
                }
            }
        }

        // Displaying k-mer distribution:
        System.out.println("Same Collision: " + sameCollision);
        System.out.println("Index Collision: " + indCollision);
        System.out.println("-----------------------------");
    ...
    public static void binarySearchTree(String sequence, int k)
    {
        // Start of timer:
        // long start = System.nanoTime();

        // Creates the Binary Search Tree with the substrings as nodes:
        BST distribution = new BST (sequence, k);

        // Initializing a variable that contains the substrings of the BST:
        ...
        for (int i = 0; i < substrings.size(); i++)
            System.out.println(substrings.get(i).getSubstring() + " (" + substrings.get(i).getOccurrences() + ")");
