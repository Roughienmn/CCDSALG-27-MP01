...
public class BST
{
    ...
    {
        // Sets the root as the initial traversing key:
        Node traverseKey = this.root;

        // Loop ends if either substring is found or reaches a leaf node:
        while (traverseKey != null)
        {
            // If substring is found to have same value as an existing node:
            if (traverseKey.getSubstring().equals(substring))
            {
                int count = traverseKey.getOccurrences();
                traverseKey.setOccurrences(count+1);
                return true;
            }
            // If substring has a value less than the parent:
            else if (traverseKey.getSubstring().compareTo(substring) > 0)
                traverseKey = traverseKey.getLeftChild();
            // If substring has a value greater than the parent:
            else
                traverseKey = traverseKey.getRightChild();
        }
